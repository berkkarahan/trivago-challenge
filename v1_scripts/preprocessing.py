...
def preparedata(df):
    ...
    df.session_durantion[df.session_durantion ==
                         0] = df.session_durantion[df.session_durantion != 0].mean()
    return df
...
def feature_engineering(df):
    ...
    sc = StandardScaler()
    df['sd_scaled'] = sc.fit_transform(
        df['session_durantion'].values.reshape(-1, 1))
    km = KMeans(n_clusters=2, init='k_means++')
    df['session_cluster'] = km.fit_transform(
        df.sd_scaled.values.reshape(-1, 1))
    return df
